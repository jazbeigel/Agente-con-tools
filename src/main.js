...
// System prompt básico, explica quien es y cual es su funcion.
const systemPrompt = ` 
...
// TODO: Implementar la Tool para buscar por nombre, LISTO
const buscarPorNombreTool = tool({
    ...
    execute: ({ nombre }) => {
        // Tu código aquí
        const buscarEstudiantePorNombre = estudiantes.buscarEstudiantePorNombre(nombre) ;
        return ;

    },
});

// TODO: Implementar la Tool para buscar por apellido, LISTO
const buscarPorApellidoTool = tool({
    ...
    execute: ({ apellido }) => {
        const buscarEstudiantePorApellido = estudiantes.buscarEstudiantePorApellido(apellido) ;
       return ;
    },
});

// TODO: Implementar la Tool para agregar estudiante, LISTO
const agregarEstudianteTool = tool({
    ...
    execute: ({ nombre, apellido, curso }) => {
        const estudianteAgregado = estudiantes.agregarEstudiante(nombre, apellido, curso);
        return;
    },
});

// TODO: Implementar la Tool para listar estudiantes, LISTO
const listarEstudiantesTool = tool({
    name: "listarEstudiantes",
    description: "Usa esta función para mostrar todos los estudiantes",
    parameters: z.object({}),
    execute: () => {
        const listaEstudiantes = estudiantes.listarEstudiantes().map((e) => e.nombre);
        console.log( "Lista es " + listaEstudiantes)
        return;
    },
});
